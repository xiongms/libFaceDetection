...
public class BoundingBoxView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void setResults(List<VisionDetRet> detRets, Float scale) {
        if (!mIsCreated) {
            return;
        }
        ...
        for (VisionDetRet detRet : detRets) {
            Rect rect = new Rect((int) (detRet.getLeft() * scale),
                    (int) (detRet.getTop() * scale),
                    (int) (detRet.getRight() * scale),
                    (int) (detRet.getBottom() * scale));
